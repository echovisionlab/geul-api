// Package artist adapts Artist persistence to shared capabilities.
package artist
...
// Projection owns the current OG pointers and bindings for Artist.
...
func (p *Projection) ReleasePending(ctx context.Context, tx *gorm.DB, target og.Target, cdnDomain string) error {
	...
	if locale == "" {
		return errs.FailedPrecondition("Artist OG target locale is required")
	}
	if err := tx.WithContext(ctx).Table("artist_translation").
	...
	return mediaasset.NewLifecycle(tx, cdnDomain).ReleaseExactPublicAssetBindings(
		ctx, target.EntityType, target.EntityID, []string{artistOGBindingKey(locale)},
	)
}
...
func (p *Projection) Complete(
	...
	result := tx.WithContext(ctx).Table("artist_translation").
		Where("entity_id = ? AND locale = ?", target.EntityID, locale).
	...
	return mediaasset.NewLifecycle(tx, cdnDomain).BindPublicAsset(ctx, mediaasset.Binding{
		AssetID: assetID, OwnerType: target.EntityType, OwnerID: target.EntityID, BindingKey: artistOGBindingKey(locale),
	})
}

func artistOGBindingKey(locale string) string { return "og:" + strings.TrimSpace(locale) }
