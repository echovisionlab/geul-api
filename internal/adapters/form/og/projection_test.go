...
func TestProjectionCompletionUsesTargetRowExistence(t *testing.T) {
	...
	seedFormProjectionReadyAsset(t, db, assetID)

	err := NewProjection().Complete(t.Context(), db, og.Target{
		EntityType: "form", EntityID: entityID, Locale: new("ko"), Kind: "locale",
	}, assetID, time.Now().UTC(), "https://cdn.example.com")
	require.NoError(t, err)
	var stored *string
	require.NoError(t, db.Table("form_translation").Select("og_asset_id").Where("entity_id = ? AND locale = 'ko'", entityID).Scan(&stored).Error)
...
func TestProjectionCompletionRejectsMissingTargetRow(t *testing.T) {
	...
	seedFormProjectionReadyAsset(t, db, assetID)
	err := NewProjection().Complete(t.Context(), db, og.Target{
		EntityType: "form", EntityID: uuid.NewString(), Locale: new("ko"), Kind: "locale",
	}, assetID, time.Now().UTC(), "https://cdn.example.com")
	require.ErrorIs(t, err, og.ErrTranslationTargetMissing)
}

func formProjectionUnitDB(t *testing.T) *gorm.DB {
	t.Helper()
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.Exec(`CREATE TABLE form_translation (
...
func seedFormProjectionReadyAsset(t *testing.T, db *gorm.DB, assetID string) {
	t.Helper()
	digest := make([]byte, 32)
