...
func TestSourceLocaleAuditPortsUseReviewedTypedBuilders(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{name: "release", entityType: "release", action: sharedtelemetry.AuditReleaseUpdated, targetType: "release"},
		{name: "artist", entityType: "artist", action: sharedtelemetry.AuditArtistUpdated, targetType: "artist"},
		{name: "label", entityType: "label", action: sharedtelemetry.AuditLabelUpdated, targetType: "label"},
