...
// LoadContentBlockMediaReferences returns active File usage for an authorized Program Event document.
func LoadContentBlockMediaReferences(ctx context.Context, db *gorm.DB, documentID uuid.UUID) ([]*contentv1.ContentBlockMediaItem, error) {
	if db == nil {
	...
	`, documentID).Scan(&rows).Error; err != nil {
		return nil, fmt.Errorf("load Program Event Content Block attachments: %w", err)
	}
