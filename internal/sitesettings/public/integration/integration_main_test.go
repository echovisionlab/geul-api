...
func TestMain(m *testing.M) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "start public SiteSettings integration suite:", err)
		os.Exit(1)
	}
	testutil.ActivateOryIntegrationSuite(suite)
	code := m.Run()
	if err := testutil.RunIntegrationSuiteCleanups(); err != nil && code == 0 {
		fmt.Fprintf(os.Stderr, "cleanup public SiteSettings integration runtime: %v\n", err)
		code = 1
	}
	testutil.DeactivateOryIntegrationSuite(suite)
	if closeErr := suite.Close(); closeErr != nil && code == 0 {
		fmt.Fprintln(os.Stderr, "close public SiteSettings integration suite:", closeErr)
		code = 1
	}
	os.Exit(code)
}
