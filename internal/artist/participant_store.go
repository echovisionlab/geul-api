...
func artistParticipantSpec() entityParticipantSpec {
	return entityParticipantSpec{
		entityName: "artist", entityTitle: "Artist", entityArticle: "an Artist", entityIDColumn: "artist_id",
		ownerTable: "artist_owner", managerTable: "artist_manager", lockRoot: lockArtistParticipantRoot,
		lockPrincipal:    lockActiveArtistPrincipal,
		mapMutationError: mapArtistParticipantMutationError,
	}
}
...
func loadEntityParticipantViews(
	...
	manageCan, err := policyv1.Artist.Manage(entityID)
	if err != nil {
		return nil, errs.Internal(err)
	}
	...
	for _, row := range rows {
		...
		if row.HasEffectiveAuthority && row.IdentityID != "" {
			actor, subjectErr := policyv1.NewAccountIdentityActor(row.IdentityID)
			if subjectErr != nil {
				return nil, errs.Internal(subjectErr)
			}
			hasEffectiveAuthority, err = spiceDB.CheckActorCan(ctx, actor, manageCan)
			if err != nil {
...
func setEntityParticipantWithAudit(ctx context.Context, db *gorm.DB, spiceDB *auth.SpiceDBClient, spec entityParticipantSpec, entityID, memberID, role string, audit entityParticipantAudit) (time.Time, error) {
	...
	_, err := authzmutation.Execute(ctx, db, spiceDB, func(tx *gorm.DB, write authzmutation.WriteRelationships) error {
		...
		if !ownerCreatedAt.IsZero() {
			action := artistAction(policyv1.Artist.ManageParticipants)
			if role != participantRoleOwner {
				action = policyv1.Artist.RemoveOwner
			}
		...
		if !managerCreatedAt.IsZero() {
			if err := requireParticipantPermission(ctx, spiceDB, spec, entityID, policyv1.Artist.ManageParticipants, principal); err != nil {
				return err
			}
...
func participantRelationshipMutations(entityID, identityID, previousRole, nextRole string) ([]policyv1.RelationshipMutation, []policyv1.RelationshipMutation, error) {
	...
	touchRole := func(role string) (policyv1.RelationshipMutation, error) {
		...
		case participantRoleOwner:
			return policyv1.Artist.TouchOwner(entityID, actor)
		case participantRoleManager:
			return policyv1.Artist.TouchManager(entityID, actor)
		default:
	...
	deleteRole := func(role string) (policyv1.RelationshipMutation, error) {
		switch role {
		case participantRoleOwner:
			return policyv1.Artist.DeleteOwner(entityID, actor)
		case participantRoleManager:
			return policyv1.Artist.DeleteManager(entityID, actor)
		default:
			return policyv1.RelationshipMutation{}, fmt.Errorf("unsupported participant role %q", role)
		}
	}
...
func removeEntityParticipantWithAudit(ctx context.Context, db *gorm.DB, spiceDB *auth.SpiceDBClient, spec entityParticipantSpec, entityID, memberID string, audit entityParticipantAudit) error {
	_, err := authzmutation.Execute(ctx, db, spiceDB, func(tx *gorm.DB, write authzmutation.WriteRelationships) error {
		...
		if ownerCount > 0 {
			if err := requireParticipantPermission(ctx, spiceDB, spec, entityID, policyv1.Artist.RemoveOwner, principal); err != nil {
				return err
			}
			totalOwners, err := countParticipantRelations(tx, spec.ownerTable, spec.entityIDColumn, entityID, "")
			if err != nil {
...
func requireParticipantPermission(ctx context.Context, spiceDB *auth.SpiceDBClient, spec entityParticipantSpec, entityID string, action artistAction, principal *auth.UserInfo) error {
